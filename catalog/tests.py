...
def _add_catalog_permissions(user):
    ...
    perms = Permission.objects.filter(
        content_type=ct,
        codename__in=("view_cameramodel", "add_cameramodel", "change_cameramodel", "delete_cameramodel"),
    )
...
class CameraModelViewTests(TestCase):
    ...
    def test_detail_ok(self):
        ...
        response = self.client.get(
            reverse("catalog:detail", kwargs={"pk": self.camera.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_without_quotes_succeeds(self):
        self.client.login(username="vendedor", password="testpass123")
        response = self.client.post(
            reverse("catalog:delete", kwargs={"pk": self.camera.pk}),
            follow=True,
        )
        self.assertEqual(response.status_code, 200)

...
class ApiTests(TestCase):
    ...
    def setUp(self):
        ...
        perms = Permission.objects.filter(content_type=ct_quote, codename__in=('add_quote', 'change_quote', 'view_quote'))
        self.user.user_permissions.add(*perms)
        self.customer = Customer.objects.create(name='Cliente API')
        self.contact = CustomerContact.objects.create(customer=self.customer, full_name='Contacto API')
        self.camera = CameraModel.objects.create(model_code='CAM-API', name='Cámara API', base_price=Decimal('400.00'))
        self.quote = Quote.objects.create(quote_number='SCP-2026-000100', customer=self.customer, contact=self.contact, sales_user=self.user, status=Quote.STATUS_ACCEPTED, currency=Quote.CURRENCY_MXN, usd_mxn_rate=Decimal('20.00'))

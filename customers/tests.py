...
def _add_customer_permissions(user):
    ...
    perms = Permission.objects.filter(
        content_type=ct,
        codename__in=("view_customer", "add_customer", "change_customer", "delete_customer"),
    )
    user.user_permissions.add(*perms)
    ct_contact = ContentType.objects.get(app_label="customers", model="customercontact")
    perms_contact = Permission.objects.filter(
        content_type=ct_contact,
        codename__in=("view_customercontact", "add_customercontact", "change_customercontact"),
    )
...
class CustomerContactModelTests(TestCase):
    ...
    def test_str_includes_contact_and_customer(self):
        contact = CustomerContact.objects.create(
            customer=self.customer, full_name="Juan Pérez"
        )
...
class CustomerContactViewTests(TestCase):
    def setUp(self):
        ...
        self.contact = CustomerContact.objects.create(
            customer=self.customer, full_name="Contacto 1"
        )

    def test_contacts_json_ok(self):
        self.client.login(username="vendedor", password="testpass123")
        response = self.client.get(
            reverse("customers:contacts", kwargs={"pk": self.customer.pk})
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()

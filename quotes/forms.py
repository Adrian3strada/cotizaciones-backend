...
class QuoteForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "usd_mxn_rate": forms.NumberInput(
                attrs={"min": 0, "step": "0.0001", "class": "form-control", "placeholder": "Ej. 20.00"}
            ),
    ...
    def clean(self):
        ...
        if currency == Quote.CURRENCY_MXN:
            if rate is None or rate <= 0:
                self.add_error(
                    "usd_mxn_rate",
                    "Indica el tipo de cambio (MXN por 1 USD) para cotizar en pesos.",
                )
        else:
...
class QuoteItemForm(forms.ModelForm):
        ...
        widgets = {
            "unit_price": forms.NumberInput(
                attrs={
                    "readonly": True,
                    "step": "0.01",
                    "class": "form-control input-readonly input-unit-price",
                }
            ),
    ...
    def __init__(self, *args, quote_currency=None, usd_mxn_rate=None, **kwargs):
        ...
        if self.instance and self.instance.pk and self.instance.camera_model_id:
            ...
            self.fields["unit_price"].initial = Quote.unit_price_from_catalog_base(
                self.instance.camera_model.base_price,
                q.currency if q else (self._quote_currency or ""),
                getattr(q, "usd_mxn_rate", None) if q else self._usd_mxn_rate,
            )
    ...
    def clean(self):
        ...
        if camera_model:
            ...
            if q is not None and getattr(q, "pk", None):
                qc = q.currency
                rate = q.usd_mxn_rate
            cleaned_data["unit_price"] = Quote.unit_price_from_catalog_base(
                camera_model.base_price,
                qc,
                rate,
            )
        return cleaned_data

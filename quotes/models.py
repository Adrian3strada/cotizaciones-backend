...
class Quote(models.Model):
    ...
    STATUS_CHOICES = [(STATUS_DRAFT, 'Borrador'), (STATUS_SENT, 'Enviada'), (STATUS_ACCEPTED, 'Aceptada'), (STATUS_REJECTED, 'Rechazada'), (STATUS_EXPIRED, 'Expirada')]
    CURRENCY_MXN = 'MXN'
    CURRENCY_USD = 'USD'
    CURRENCY_CHOICES = [(CURRENCY_MXN, 'MXN'), (CURRENCY_USD, 'USD')]
    quote_number = models.CharField('Número', max_length=20, unique=True, editable=False)
    customer = models.ForeignKey(Customer, verbose_name='Cliente', on_delete=models.PROTECT, related_name='quotes')
    contact = models.ForeignKey(CustomerContact, verbose_name='Contacto', on_delete=models.SET_NULL, related_name='quotes', null=True, blank=True)
    sales_user = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name='Vendedor', on_delete=models.PROTECT)
    status = models.CharField('Estatus', max_length=20, choices=STATUS_CHOICES, default=STATUS_DRAFT)
    ...
    usd_mxn_rate = models.DecimalField('Tipo de cambio (MXN por 1 USD)', max_digits=12, decimal_places=4, null=True, blank=True, help_text='Obligatorio si la moneda es MXN. Los precios de lista del catálogo están en USD.')
    subtotal = models.DecimalField('Subtotal', max_digits=14, decimal_places=2, default=Decimal('0.00'))
    special_discount_percent = models.DecimalField('Descuento especial (%)', max_digits=5, decimal_places=2, default=Decimal('0.00'), help_text='Porcentaje de descuento especial aplicado al total de productos (todos los clientes).')
    special_discount_amount = models.DecimalField('Descuento especial (monto)', max_digits=14, decimal_places=2, default=Decimal('0.00'))
    tax_rate = models.DecimalField('IVA (%)', max_digits=5, decimal_places=2, default=Decimal('16.00'))
    ...
    cableado_monto = models.DecimalField('Cableado (monto)', max_digits=14, decimal_places=2, default=Decimal('0.00'), blank=True)
    instalacion = models.BooleanField('Instalación', default=False)
    instalacion_monto = models.DecimalField('Instalación (monto)', max_digits=14, decimal_places=2, default=Decimal('0.00'), blank=True)
    poe = models.BooleanField('PoE', default=False)
    poe_monto = models.DecimalField('PoE (monto)', max_digits=14, decimal_places=2, default=Decimal('0.00'), blank=True)
    notes = models.TextField('Observaciones', blank=True)
    terms = models.TextField('Términos', blank=True, default='Entrega 10-15 días hábiles. Pago 50% anticipo, 50% contra entrega. Garantía 1 año.')
    ...
    class Meta:
        ...
        ordering = ['-created_at']
        indexes = [models.Index(fields=['sales_user', 'status']), models.Index(fields=['issue_date']), models.Index(fields=['customer', 'status'])]
    ...
    def clean(self) -> None:
        ...
        if discount_pct is not None and (discount_pct < 0 or discount_pct > 100):
            raise ValidationError({'special_discount_percent': 'El descuento especial debe ser entre 0 y 100%.'})
    ...
    @staticmethod
    def unit_price_from_catalog_base(base_price: Decimal, quote_currency: str, usd_mxn_rate: Decimal | None) -> Decimal:
    ...
    def generate_quote_number(self) -> str:
        ...
        prefix = f'SCP-{year}-'
        last_quote = Quote.objects.filter(quote_number__startswith=prefix).select_for_update().order_by('-quote_number').first()
        next_number = 1
        if last_quote:
    ...
    def get_optional_rows(self) -> list[dict]:
        ...
        m = getattr(self, 'cableado_monto', None) or Decimal('0.00')
        if getattr(self, 'cableado', False) and m > 0:
            rows.append({'partida': base_partida + len(rows) + 1, 'desc': 'Cableado', 'monto': m})
        m = getattr(self, 'instalacion_monto', None) or Decimal('0.00')
        if getattr(self, 'instalacion', False) and m > 0:
            rows.append({'partida': base_partida + len(rows) + 1, 'desc': 'Instalación', 'monto': m})
        m = getattr(self, 'poe_monto', None) or Decimal('0.00')
        if getattr(self, 'poe', False) and m > 0:
            rows.append({'partida': base_partida + len(rows) + 1, 'desc': 'PoE', 'monto': m})
        return rows
    ...
    def recalculate_totals(self) -> None:
        ...
        self.special_discount_amount = (subtotal * discount_pct / Decimal('100')).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        ...
        try:
            tax_amount = (base_for_iva * tax_rate / Decimal('100')).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        ...
        if self.pk is not None:
            Quote.objects.filter(pk=self.pk).update(subtotal=self.subtotal, special_discount_amount=self.special_discount_amount, tax_amount=self.tax_amount, total=self.total)

class QuoteItem(models.Model):
    quote = models.ForeignKey(Quote, verbose_name='Cotización', on_delete=models.CASCADE, related_name='items')
    camera_model = models.ForeignKey(CameraModel, verbose_name='Modelo de cámara', on_delete=models.PROTECT)
    quantity = models.PositiveIntegerField('Cantidad')
    unit_price = models.DecimalField('Precio unitario', max_digits=14, decimal_places=2)
    discount_percent = models.DecimalField('Descuento (%)', max_digits=5, decimal_places=2, default=Decimal('0.00'))
    discount_amount = models.DecimalField('Descuento (monto)', max_digits=14, decimal_places=2, default=Decimal('0.00'))
    line_subtotal = models.DecimalField('Subtotal línea', max_digits=14, decimal_places=2, default=Decimal('0.00'))
    configuration_notes = models.TextField('Notas de configuración', blank=True)
    group_name = models.CharField('Grupo', max_length=120, blank=True, help_text='Opcional. Reservado para API/admin; detalle web y PDF listan ítems en orden plano.')
    order_in_group = models.PositiveSmallIntegerField('Orden en grupo', default=0, help_text='Opcional. Sin efecto en vista o PDF actuales.')
    ...
    def clean(self) -> None:
        ...
        if discount_pct is not None and (discount_pct < 0 or discount_pct > 100):
            raise ValidationError({'discount_percent': 'El descuento debe ser un porcentaje entre 0 y 100.'})
    ...
    def save(self, *args, **kwargs):
        ...
        if self.camera_model_id:
            ...
            self.unit_price = Quote.unit_price_from_catalog_base(self.camera_model.base_price, q.currency if q else '', getattr(q, 'usd_mxn_rate', None) if q else None)
        ...
        self.discount_amount = (line_before_discount * discount_pct / Decimal('100')).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)

...
def _customer_quote_currency_json():
    return json.dumps({str(c.pk): c.default_quote_currency() for c in Customer.objects.only('id', 'country_code')})
...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        totals = queryset.aggregate(total_amount=Coalesce(Sum('total'), Value(0), output_field=DecimalField(max_digits=14, decimal_places=2)), accepted_count=Count('id', filter=models.Q(status=Quote.STATUS_ACCEPTED)), sent_count=Count('id', filter=models.Q(status=Quote.STATUS_SENT)), draft_count=Count('id', filter=models.Q(status=Quote.STATUS_DRAFT)), rejected_count=Count('id', filter=models.Q(status=Quote.STATUS_REJECTED)), expired_count=Count('id', filter=models.Q(status=Quote.STATUS_EXPIRED)), total_quotes=Count('id'))
        accepted_total = accepted_queryset.aggregate(total=Coalesce(Sum('total'), Value(0), output_field=DecimalField(max_digits=14, decimal_places=2)))['total']
        conversion_rate = 0
        if totals['total_quotes']:
            conversion_rate = round(totals['accepted_count'] / totals['total_quotes'] * 100, 2)
        top_customer = accepted_queryset.values('customer__name').annotate(total=Sum('total')).order_by('-total').first()
        top_seller = accepted_queryset.values('sales_user__username').annotate(total=Sum('total')).order_by('-total').first()
        top_models = list(QuoteItem.objects.filter(quote__in=accepted_queryset).values('camera_model__model_code', 'camera_model__name').annotate(total_qty=Coalesce(Sum('quantity'), Value(0))).order_by('-total_qty')[:6])
        ...
        monthly_totals = queryset.filter(issue_date__gte=start_six_months.replace(day=1), issue_date__lte=current_date).annotate(year=ExtractYear('issue_date'), month=ExtractMonth('issue_date')).values('year', 'month').annotate(total=Coalesce(Sum('total'), Value(0), output_field=DecimalField(max_digits=14, decimal_places=2))).order_by('year', 'month')
        monthly_map = {(row['year'], row['month']): row['total'] for row in monthly_totals}
        last_months = []
        for offset in range(5, -1, -1):
            d = current_date - relativedelta(months=offset)
            last_months.append({'label': month_labels[d.month - 1][:3], 'total': monthly_map.get((d.year, d.month), 0)})
        ...
        prev_year_accepted = queryset.filter(status=Quote.STATUS_ACCEPTED, issue_date__year=prev_year).aggregate(total=Coalesce(Sum('total'), Value(0), output_field=DecimalField(max_digits=14, decimal_places=2)))['total']
        year_vs_prev = None
        ...
        chart_status_data = [totals['draft_count'], totals['sent_count'], totals['accepted_count'], totals['rejected_count'], totals['expired_count']]
        ...
        context.update({'customer_count': total_customers, 'customers_with_quotes': customers_with_quotes, 'model_count': CameraModel.objects.count(), 'quote_count': totals['total_quotes'], 'total_amount': totals['total_amount'], 'accepted_total': accepted_total, 'accepted_count': totals['accepted_count'], 'sent_count': totals['sent_count'], 'draft_count': totals['draft_count'], 'rejected_count': totals['rejected_count'], 'expired_count': totals['expired_count'], 'conversion_rate': conversion_rate, 'top_customer': top_customer, 'top_seller': top_seller, 'top_models': top_models, 'last_months': last_months, 'year_vs_prev': year_vs_prev, 'active_customer_rate': active_customer_rate, 'today_label': current_date.strftime('%d/%m/%Y'), 'chart_status_labels': chart_status_labels, 'chart_status_data': chart_status_data, 'chart_models_labels': chart_models_labels, 'chart_models_data': chart_models_data, 'chart_months_labels': chart_months_labels, 'chart_months_data': chart_months_data})
        return context
...
class QuoteListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        if _user_sees_only_own_quotes(self.request):
            context['customers'] = Customer.objects.filter(quotes__sales_user=self.request.user).distinct().values_list('id', 'name').order_by('name')
            context['sales_users'] = get_user_model().objects.filter(id=self.request.user.id).values_list('id', 'username')
        else:
        ...
        context['allowed_order'] = {'issue_date', '-issue_date', 'quote_number', '-quote_number', 'total', '-total', 'valid_until', '-valid_until', 'customer__name', '-customer__name', 'sales_user__username', '-sales_user__username', 'currency', '-currency'}
        return context
...
class QuoteDetailView(LoginRequiredMixin, PermissionRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        queryset = super().get_queryset().select_related('customer', 'contact', 'sales_user').prefetch_related('items__camera_model')
        if _user_sees_only_own_quotes(self.request):
            queryset = queryset.filter(sales_user=self.request.user)
        return queryset
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        quote = self.object
        context['is_expired'] = quote.status == Quote.STATUS_EXPIRED or (quote.valid_until and quote.valid_until < timezone.localdate())
        return context
...
def _get_quote_list_queryset(request):
    ...
    if search:
        queryset = queryset.filter(Q(quote_number__icontains=search) | Q(customer__name__icontains=search))
    if status:
    ...
    if expiring_days:
        try:
            ...
            if days > 0:
                ...
                queryset = queryset.filter(status__in=[Quote.STATUS_DRAFT, Quote.STATUS_SENT], valid_until__gte=today, valid_until__lte=limit)
        except (TypeError, ValueError):
            pass
    order_by = request.GET.get('order_by', '-issue_date')
    allowed = {'issue_date', '-issue_date', 'quote_number', '-quote_number', 'total', '-total', 'valid_until', '-valid_until', 'customer__name', '-customer__name', 'sales_user__username', '-sales_user__username', 'currency', '-currency'}
...
def quote_create(request):
    ...
    optional_service_fields = {'cableado', 'cableado_monto', 'instalacion', 'instalacion_monto', 'poe', 'poe_monto'}
    camera_model_prices = json.dumps({str(pk): str(price) for pk, price in CameraModel.objects.values_list('id', 'base_price')})
    return render(request, 'quotes/quote_form.html', {'form': form, 'formset': formset, 'is_edit': False, 'optional_service_fields': optional_service_fields, 'camera_model_prices': camera_model_prices, 'customer_quote_currency_json': _customer_quote_currency_json(), 'default_usd_mxn_rate': _default_usd_mxn_rate_display()})
...
def quote_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        formset = QuoteItemFormSet(request.POST, instance=quote, **_item_formset_quote_args(request, quote=quote))
        if form.is_valid() and formset.is_valid():
    ...
    optional_service_fields = {'cableado', 'cableado_monto', 'instalacion', 'instalacion_monto', 'poe', 'poe_monto'}
    camera_model_prices = json.dumps({str(pk): str(price) for pk, price in CameraModel.objects.values_list('id', 'base_price')})
    return render(request, 'quotes/quote_form.html', {'form': form, 'formset': formset, 'is_edit': True, 'quote': quote, 'optional_service_fields': optional_service_fields, 'camera_model_prices': camera_model_prices, 'customer_quote_currency_json': _customer_quote_currency_json(), 'default_usd_mxn_rate': _default_usd_mxn_rate_display()})
...
def quote_duplicate(request, pk):
    quote = _get_quote_for_user(request, pk)
    with transaction.atomic():
        new_quote = Quote(quote_number='', customer=quote.customer, contact=quote.contact, sales_user=request.user, status=Quote.STATUS_DRAFT, currency=quote.currency, usd_mxn_rate=quote.usd_mxn_rate, tax_rate=quote.tax_rate, special_discount_percent=quote.special_discount_percent, cableado=quote.cableado, cableado_monto=quote.cableado_monto or Decimal('0.00'), instalacion=quote.instalacion, instalacion_monto=quote.instalacion_monto or Decimal('0.00'), poe=quote.poe, poe_monto=quote.poe_monto or Decimal('0.00'), notes=quote.notes or '', terms=quote.terms or '')
        new_quote.save()
        for item in quote.items.all():
            QuoteItem.objects.create(quote=new_quote, camera_model=item.camera_model, quantity=item.quantity, unit_price=item.unit_price, discount_percent=item.discount_percent, group_name=item.group_name or '', order_in_group=item.order_in_group, configuration_notes=item.configuration_notes or '')
...
def quote_pdf(request, pk):
    ...
    issue_date_formatted = f'{quote.issue_date.day} {_meses[quote.issue_date.month - 1]} {quote.issue_date.year}' if quote.issue_date else ''
    ...
    if not use_reportlab:
        try:
            ...
            header_right_path = finders.find(getattr(django_settings, 'QUOTE_PDF_HEADER_IMAGE', 'img/quote_header_right.png'))
            logo_uri = _build_file_uri(logo_path)
            header_right_uri = _build_file_uri(header_right_path)
            pdf_context = {'quote': quote, 'vigencia_texto': vigencia_texto, 'issue_date_formatted': issue_date_formatted, 'company_name': company.get('name', 'Sistemas de Conteo de Personas.'), 'company_website': company.get('website', 'www.sisconper.com'), 'company_street': company.get('street', 'Blvd. Paseo de la República No. 13020 Int. 1307'), 'company_colony': company.get('colony', 'Col. Juriquilla, Querétaro, Qro.'), 'company_postal_code': company.get('postal_code', 'C.P. 76230'), 'company_phone': company.get('phone', '(442) 245 7000'), 'company_mobile': company.get('mobile', ''), 'company_rfc': company.get('rfc', 'SCP070410C43'), 'company_email': company.get('email', 'info@sisconper.com'), 'company_logo_uri': logo_uri, 'header_right_uri': header_right_uri}
            ...
            messages.error(request, 'No se pudo generar el PDF. Por favor, intente de nuevo o contacte al administrador.')
            return redirect('quotes:detail', pk=pk)
...
def report_view(request):
    ...
    monthly_counts = queryset.annotate(month=ExtractMonth('issue_date')).values('month', 'status').annotate(total=Count('id')).order_by('month')
    status_counts = queryset.values('status').annotate(total=Count('id')).order_by('status')
    accepted_queryset = queryset.filter(status=Quote.STATUS_ACCEPTED)
    top_models = QuoteItem.objects.filter(quote__in=accepted_queryset).values('camera_model__model_code', 'camera_model__name').annotate(total_qty=Sum('quantity')).order_by('-total_qty')[:5]
    totals = queryset.aggregate(total_quotes=Count('id'), total_amount=Coalesce(Sum('total'), Value(0), output_field=DecimalField(max_digits=14, decimal_places=2)), accepted_count=Count('id', filter=models.Q(status=Quote.STATUS_ACCEPTED)))
    conversion_rate = 0
    if totals['total_quotes']:
        conversion_rate = round(totals['accepted_count'] / totals['total_quotes'] * 100, 2)
    month_labels = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre']
    ...
    for row in monthly_counts:
        ...
        monthly_rows.append({'month': month_value, 'month_label': month_label_map.get(month_value, month_value), 'status': row['status'], 'status_label': status_label_map.get(row['status'], row['status']), 'total': row['total']})
    status_totals = []
    status_map = {row['status']: row['total'] for row in status_counts}
    for status_key in status_keys:
        status_totals.append(status_map.get(status_key, 0))
    return render(request, 'quotes/report.html', {'monthly_counts': monthly_counts, 'monthly_rows': monthly_rows, 'status_counts': status_counts, 'top_models': top_models, 'year': current_year, 'totals': totals, 'conversion_rate': conversion_rate, 'customers': Customer.objects.filter(quotes__in=queryset).distinct().values_list('id', 'name').order_by('name'), 'sales_users': get_user_model().objects.filter(pk__in=queryset.values_list('sales_user_id', flat=True).distinct()).values_list('id', 'username').order_by('username'), 'month_labels': month_labels, 'status_labels': status_labels, 'status_keys': status_keys, 'series': series, 'status_totals': status_totals})

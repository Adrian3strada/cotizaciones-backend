...
_ITEM_PATTERNS = [[(0, 3), (1, 2)], [(0, 5), (2, 2)], [(1, 4), (3, 1)], [(2, 6), (4, 2)], [(0, 2), (5, 3)], [(3, 8), (1, 2)], [(4, 3), (6, 4)], [(0, 10)], [(7, 2), (8, 2)], [(2, 5), (9, 1)], [(1, 3), (4, 3)], [(6, 4), (10, 2)]]
_STATUS_CYCLE = [Quote.STATUS_DRAFT, Quote.STATUS_SENT, Quote.STATUS_ACCEPTED, Quote.STATUS_SENT, Quote.STATUS_REJECTED, Quote.STATUS_DRAFT, Quote.STATUS_ACCEPTED, Quote.STATUS_EXPIRED, Quote.STATUS_SENT]
_DISCOUNTS = [None, Decimal('5.00'), None, Decimal('10.00'), None, Decimal('8.00'), Decimal('3.00'), None]

class Command(BaseCommand):
    help = 'Crea cotizaciones de ejemplo (notes=SEED_QUOTES) repartidas desde el 1 de enero del año en curso hasta hoy. Requiere clientes con contactos y modelos en catálogo.'

    def add_arguments(self, parser):
        parser.add_argument('--reset', action='store_true', help='Elimina antes las cotizaciones SEED_QUOTES existentes.')
    ...
    def handle(self, *args, **options):
        if options.get('reset'):
            ...
            self.stdout.write(self.style.WARNING('Ya existen cotizaciones SEED_QUOTES. Pasa --reset para regenerarlas desde enero.'))
            return
        User = get_user_model()
        staff_users = list(User.objects.filter(is_staff=True).order_by('id'))
        if not staff_users:
            user = User.objects.create_user(username='ventas', password='ventas123', is_staff=True, email='ventas@ejemplo.com')
            staff_users = [user]
            self.stdout.write(self.style.WARNING("Creado usuario staff 'ventas' para asignar cotizaciones."))
        customers = list(Customer.objects.filter(contacts__isnull=False).distinct().order_by('name')[:20])
        ...
        for i in range(num):
            ...
            quote = Quote(customer=customer, contact=contact, sales_user=sales_user, issue_date=issue_date, valid_until=valid_until, currency=Quote.CURRENCY_MXN, usd_mxn_rate=tc, tax_rate=Decimal('16.00'), special_discount_percent=discount or Decimal('0.00'), notes='SEED_QUOTES', terms=terms)
            opt = i % 9
            if opt in (1, 4, 7):
                quote.cableado = True
            ...
            for cam_idx, qty in pattern:
                ...
                QuoteItem.objects.create(quote=quote, camera_model=cam, quantity=qty, unit_price=cam.base_price)
            ...
            created += 1
        self.stdout.write(self.style.SUCCESS(f'Creadas {created} cotizaciones SEED_QUOTES ({year_start.isoformat()} -> {today.isoformat()}).'))
